...
| 路径 | 用途 |
| --- | --- |
| `eeglab_mcp_server/` | 可执行 stdio MCP server、工具 schema、handlers、registry 和官方对齐 map。 |
| `skills/eeglab-analysis/` | 研究工作流 Skill，以及面向 agent 客户端的聚焦参考文件。 |
| `docs/` | 官方覆盖、支持等级、风险、工作流和报告字段矩阵。 |
| `configs/` | Codex、Claude Desktop、Cursor、VS Code 和通用客户端的 MCP 配置模板。 |
| `scripts/` | setup、doctor、uninstall 和 verification 入口脚本。 |

日常使用从 `README.md` 或本文件开始，运行 `scripts/eeglab_agent.ps1 setup`，再运行 `scripts/eeglab_agent.ps1 verify` 和 `scripts/eeglab_agent.ps1 doctor`。涉及科学支持判断时，优先查看 `docs/` 中的官方矩阵，或使用对应 `eeglab://official/...` MCP resources。
...
`scripts/eeglab_agent.ps1` 是最短的用户入口。它只转发到专用 setup、verify、doctor 和 uninstall 脚本，所以自动化流程仍然可以直接调用底层脚本。
...
统一 verifier 会编译 server/verifier Python 文件、验证 MCP framework，并检查官方对齐 contract：
