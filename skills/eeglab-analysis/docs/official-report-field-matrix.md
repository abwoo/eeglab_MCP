...
- connectivity threshold strategy, stability checks, and interpretation limits for sensor or ROI networks
...
- required figure families for the active branch
- conditional figure families for justified secondary evidence
- guidance-only figure families that are indexed but not executable by default
- figure paths and figure descriptions
- figure grouping/ROI/context notes for ERP, PSD, ERSP/ITC, component review, connectivity, source, and STUDY figures
- ERP-image or single-trial visualization notes when the workflow uses them as indexed-only or report-only evidence
- figure atlas entries with figure type, status, branch, channel/ROI, latency/frequency window, tool source, interpretation scope, and figure path
...
- official method profile ID, gate status, source claim IDs, missing requirements, and `missing_requirements`
- override status, override reason, and blocked requirements acknowledged
- branch workflow coverage: branch ID, branch label, branch mode, analysis type,
  branch variant, universal preamble steps, ordered branch steps, completed
  steps, missing required steps, blocked steps, required figures, figure paths,
  required outputs, output paths, required figure families, conditional figure families, guidance-only figure families,
  branch_workflow, branch_completeness, and completeness status
